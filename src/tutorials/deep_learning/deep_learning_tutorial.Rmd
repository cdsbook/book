...
knitr::opts_knit$set(root.dir = str_c(here::here(),"/src/tutorials/deep_learning"))
...
This tutorial will introduce you to **deep learning**.
...
Instead of cross-validation, we will simply take a fixed subset of the training images to be the validation set, which we can do with the `dataset_subset()` function from the `torch` package. Here we are selecting the first 10000 images to be our validation set:
