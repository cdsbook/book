---
title: "Relational Data and Databases tutorial"
author: 
 - "Dominic White"
tutorial:
  version: 1.1
...
Sometimes a dataset might be split across multiple tables of data (i.e. multiple dataframes). In such cases, there are often columns in each table that link the two tables back together.
...
A __database__ is an organized way of storing data.

A particularly common form of database is the **relational database**, in which data is broken up into multiple tables. In general the goal of a relational database is to avoid repeating the same piece of data in multiple rows.
...
We typically interact with databases via a software programm called a *database management system*. This program makes sure that the data is organized properly, and allows us to access or modify the data.

Relational databases have a special programming language for accessing their data. This programming language is called __SQL__.

> SQL stands for Structured Query Language. It is properly pronounced using by spelling out the acronym: "ess-kew-ell". However, a lot of people prnounce it as "sequel".

Fortunately we do not need to learn SQL for this class (although you should learn SQL in the future if you wish to do more work with data). Instead we can interact with relational databases using the data wrangling functions that we have already learned.
