...
1. The first line creates the function almost like a new variable! 

   i. We put the name for the new function (e.g. `function_name`) on the left-hand side of the assignment operator `<-`.
   ii. Then on the right-hand side of the assignment operator we use the `function` *keyword* to indicate that we are creating a function instead of a normal variable. `function` is immediately followed by parentheses `(...)`.
   iii. Inside the parentheses we put the names of any parameters that the function will accept as inputs e.g. `parameter_1`. Each parameter name is separated by a comma.
   iv. We end the line with a curly brace `{`. This indicates that all code after this is part of the function, until we reach a matching closing curly brace `}`.
