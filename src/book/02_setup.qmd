...
* If you have any interest in working in tech in the future, then pick something vaguely professional as your GitHub username. You don't want to have to explain to a future employer why your GitHub username is `squeaky_boi69`. Your GitHub profile might be the programming equivalent of your LinkedIn profile.
...
* They will also ask you a bunch of questions when you sign up about what you want to use GitHub for. It really doesn't matter what you respond to these questions (you will get the same account at the end), so either answer them or skip through them.


## Step 2 (Option 1): Setting up RStudio Desktop

If you are installing RStudio Desktop on your computer, then you should follow these steps:

1. First, you should first install Git by following the appropriate installation instructions for your operating system on the Git website: https://git-scm.com/downloads
...
3. Finally we are can install RStudio Desktop. Download the installer for your operating system here and then install from it: https://posit.co/download/rstudio-desktop/
...
4. One last step is to store your GitHub account details in RStudio. There are a few steps to this, so follow along carefully.
  ...
  * Then go to the _Git_ tab of the Options pop-up and type in your name (probably either your full name or your GitHub username) and the __same__ email you used to register for GitHub (it is important that these emails match), and then click Save.
  ...
  * Finally, you can also go to the _Editors_ tab in the options and pick RStudio as your default editor (this is optional, since we won't need to use GitHub Desktop ever again in this book!).
  ...
  __Linux users:__ 
    ...
    * First, go to RStudio and open the tab in the left hand pane called *Terminal*. If you do not see a Terminal tab, then you can create one from the top menu of RStudio Desktop by going to "Tools > Terminal > New Terminal".
      ...
      Then run this line, again making sure to replace the email inside the quotation marks with the same email you used to sign up for GitHub:
      
      ```bash
      git config --global user.email "you@emailHost.com"
      ```
      
      I would also recommend running one final line in the Terminal (this will enable your computer to store your GitHub login details).
    ...
    * Then go to the Console tab (which should be next to the Terminal), and copy and paste these lines of R code one at a time:
      ...
      On the token webpage that appears, I recommend the following options:
      
      * In the *Note* field, write `RStudio`.
      * For the expiration date, pick a date about 4 months into the future (or after the end of the semester if you are using this for a class).
      
      You can leave all the checked *scopes* as the defaults, and then scroll down to the green button at the bottom and click *Generate token*.
...
## Option 2: Setting up Posit Cloud

Got to https://posit.cloud/ and create an account. You can sign up for the free plan, which at the time of writing includes 25 hours per month before they ask you to pay.
...
Find the line on this page where it says GitHub, and click any unchecked checkboxes.

This may open up new webpages taking you to GitHub, which ask you to verify that you want to authorize Posit to access your GitHub account. Make sure you agree all of these, otherwise you might not be able to download or upload code from/to GitHub.
